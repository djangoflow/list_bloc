...
void main() {
  group('ListCubit', () {
    ...
    group('removeLocally tests', () {
      blocTest<ListCubit<Item, ItemType>, Data<List<Item>, ItemType>>(
        'is able to remove item when item is present',
        build: () => cubit,
      ...
      blocTest<ListCubit<Item, ItemType>, Data<List<Item>, ItemType>>(
        'emits empty state when list is empty after remove',
        build: () => cubit,
        seed: () => Data<List<Item>, ItemType>(
          data: [Item(type: ItemType.fruit, value: 'Apple')],
        ),
      ...
      blocTest<ListCubit<Item, ItemType>, Data<List<Item>, ItemType>>(
        'emits type of original state when list is not empty after remove',
        build: () => cubit,
    ...
    group('add locally tests', () {
      blocTest<ListCubit<Item, ItemType>, Data<List<Item>, ItemType>>(
        ...
        act: (bloc) => bloc.addLocally(
          Item(type: ItemType.vegetable, value: 'Potato'),
        ),
        expect: () => [
          isA<Data<List<Item>, ItemType>>().having(
            (state) => state.data?.where((e) => e.value == 'Potato').isNotEmpty,
      ...
      blocTest<ListCubit<Item, ItemType>, Data<List<Item>, ItemType>>(
        'emits data state if list was empty before add',
        build: () => cubit,
        seed: () => Empty<List<Item>, ItemType>(),
        act: (bloc) => bloc.addLocally(
          Item(type: ItemType.vegetable, value: 'Potato'),
        ),
        expect: () => [
